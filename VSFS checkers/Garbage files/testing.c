...
#define MAX_INODES (5 * BLOCK_SIZE / INODE_SIZE)  // 5 blocks * 4096 bytes / 256 bytes
...
int *block_refs = NULL;        // Count references to each block
bool *visited_blocks = NULL;   // Track visited blocks for cycles
...
int main(int argc, char *argv[]) {
    ...
    // Write the corrected file system image
...
bool load_fs_image(const char *filename) {
  ...
  //      fprintf(stderr, "Invalid file size: expected %d bytes, got %ld bytes\n", 
    ...
    // Set up pointers to various parts of the file system
    ...
    // Allocate memory for computed bitmaps and other data structures
...
void check_data_bitmap() {
    ...
    // Reset the computed data bitmap and block references
    ...
    // Reserved blocks are always used (superblock, bitmaps, inode table)
    for (uint32_t i = 0; i < DATA_BLOCK_START_IDX; i++) {
        block_refs[i] = 1;  // Mark as referenced once
    }
    
    // Process all valid inodes to find data block references
    for (uint32_t i = 0; i < superblock->inode_count; i++) {
    ...
    // Compare with actual data bitmap
...
void check_bad_blocks() {
    ...
    // Check all valid inodes for out-of-range block references
    for (uint32_t i = 0; i < superblock->inode_count; i++) {
        if (!is_valid_inode(i)) continue;
